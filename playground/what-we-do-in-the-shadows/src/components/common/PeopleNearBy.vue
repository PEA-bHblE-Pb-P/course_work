<template>
  ...
  <div :class="{ hidden: !isBar || !selected }" class="m-2">
    <button
        ...
        @click="drinkBlood"
    >DRINK BLOOD</button>
    <span class="font-medium block">Blood amount: <input
        v-model.trim="amount"
        maxlength="3"
        class="
            mb-4
  ...
  <div class="flex flex-wrap">
    <Character v-for="person in people" :key="person.id" :character="person" :highlight="person.id === selected"
...
export default {
  ...
  async mounted() {
    ...
    this.isBar = mapLocationType(this.$store.state.character.location) === 'bar'
